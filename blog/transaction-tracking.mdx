...
description: A comprehensive guide on tracking transactions in TON blockchain using normalized message hashes and trace lifecycle states.
author: TON API Team
date: 2025-01-06
---
...
<Tabs items={['TypeScript', 'Go (tongo)', 'Go (tonutils-go)', 'Python (tonutils)']}>
...
For a more detailed, step-by-step example of tracking a transaction using normalized hashes, check out the [Cookbook](/docs/tonapi/cookbook/transaction-tracking).
...
This BOC contains everything you need to get the exact same normalized hash, even before the message appears on-chain. See how to decode and handle BOC strings in the [Cookbook](/docs/tonapi/cookbook/transaction-tracking#from-a-boc).
